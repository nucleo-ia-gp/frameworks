# Núcleo de IA em Gestão de Projetos — Frameworks

Original frameworks, methodologies, and research outputs from the Núcleo de IA em Gestão de Projetos, a multi-chapter research program under PMI Brazil.

## License

- Documentation and methodologies: [CC-BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)
- Code and tools: [MIT](LICENSE)

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines on proposing, reviewing, and publishing frameworks.

## Frameworks

| Framework | Tribe | Status | Lead |
|-----------|-------|--------|------|
| EAA — Engenharia de Agentes Autônomos | Tribo 2 | In development (not yet published) | Débora Moura |

> Frameworks are published here when the responsible tribe approves public release.

## Structure

Each framework lives in its own directory:

```
framework-name/
  README.md       Overview, citation, status
  docs/           Documentation (CC-BY-SA 4.0)
  examples/       Reference implementations (MIT)
  references/     Comparative reviews and bibliography
```
